...
class TipScrapper extends BaseScrapper {
    ...
    private function scrapByJson($callback) {
        ...
        if (!empty($result) && isset($result['feedback']) && !empty($result['feedback'])) {
            ...
            foreach ($result['feedback'] as $key => $record) {
                $tips[$key] = $this->makePostArray($record);
            }
            ...
            if(isset($result['isComplete']) && empty($result['isComplete'])) {
                ...
                if(!empty($cursor)) {
                    ...
                    $urlToScrap = "https://www.khanacademy.org/api/internal/discussions/video/{$nodeSlug}/comments?casing=camel&sort=1&subject=all&limit=50&page=0&cursor={$cursor}&lang=en";
